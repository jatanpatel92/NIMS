...
include '../ofc/php-ofc-library/open-flash-chart.php';
    ...
    $title = new title( "Member Job Card Status<br>" );
    $title->set_style( "{font-size: 20px; font-family: Times New Roman; font-weight: bold; color: #C35617; text-align: center;}" );
    $query="Select com_name, Count(*) as cnt1 from member_info natural join family_info natural join community_info where
    mem_jobcard_status=\"ISSUED\" GROUP BY com_name";
    ...
    $query="Select com_name, Count(*) as cnt1 from member_info natural join family_info natural join community_info where
    mem_jobcard_status=\"APPLIED_NOT_ISSUED\" GROUP BY com_name";
    $result=run_query_2($query);
    $data2 = array();
    $bar2 = new bar_glass();
    $bar2->colour( '#5E0722');
    ...
    $query="Select com_name, Count(*) as cnt1 from member_info natural join family_info natural join community_info where
    mem_jobcard_status=\"NOT_APPLIED\" GROUP BY com_name";
    $result=run_query_2($query);
    $data3 = array();
    $bar3 = new bar_glass();
    $bar3->colour( '#432BAF');

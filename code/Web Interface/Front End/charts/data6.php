...
include '../ofc/php-ofc-library/open-flash-chart.php';  
        ...
        $query="(Select Count(*) as cnt1 from member_info natural join family_info where mem_voterstatus=\"ISSUED\" AND com_id=".$id."); ";
        $result=run_query_2($query);
        $data = array();
        $row = mysql_fetch_array($result);
        
        $query="(Select Count(*) as cnt2 from member_info natural join family_info where mem_voterstatus=\"APPLIED_NOT_ISSUED\" AND com_id=".$id.");";
        $result2=run_query_2($query);
        $row2 = mysql_fetch_array($result2);
        
        $query="(Select Count(*) as cnt3 from member_info natural join family_info where mem_voterstatus=\"NOT_APPLIED\" AND com_id=".$id."); ";
        ...
        $data[] = new pie_value(intval($row3['cnt3']),('Not Applied'));          
         ...
         $pie->set_colours(
          ...
          $pie->set_values( $data );
          $title = new title( 'Voter Card Status' );

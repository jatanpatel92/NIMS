...
include '../ofc/php-ofc-library/open-flash-chart.php';  
        ...
        $query="(Select Count(*) as cnt1 from family_info where fam_ration_card_status=\"ISSUED\" AND com_id=".$id."); ";
        $result=run_query_2($query);
        $data = array();
        $row = mysql_fetch_array($result);
        
        $query="(Select Count(*) as cnt2 from family_info where fam_ration_card_status=\"APPLIED_NOT_ISSUED\" AND com_id=".$id."); ";
        $result2=run_query_2($query);
        $row2 = mysql_fetch_array($result2);
        
        $query="(Select Count(*) as cnt3 from family_info where fam_ration_card_status=\"NOT_APPLIED\" AND com_id=".$id."); ";
        ...
        $data[] = new pie_value(intval($row3['cnt3']),('Not Applied'));          
         ...
         $pie->set_colours(
          ...
          $pie->set_values( $data );
          $title = new title( 'Ration Card Status' );
